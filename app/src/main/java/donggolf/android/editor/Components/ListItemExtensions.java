...
import static donggolf.android.editor.Components.InputExtensions.CONTENT;
...
public class ListItemExtensions extends EditorComponent {
    ...
    public View addListItem(TableLayout layout, boolean isOrdered, String text) {
        ...
        final donggolf.android.editor.Components.CustomEditText editText = childLayout.findViewById(R.id.txtText);
    ...
    public void convertListToOrdered(TableLayout _table) {
        ...
        for (int i = 0; i < _table.getChildCount(); i++) {
            View _childRow = _table.getChildAt(i);
            donggolf.android.editor.Components.CustomEditText editText = (donggolf.android.editor.Components.CustomEditText) _childRow.findViewById(R.id.txtText);
    ...
    public void convertListToUnordered(TableLayout _table) {
        ...
        for (int i = 0; i < _table.getChildCount(); i++) {
            View _childRow = _table.getChildAt(i);
            donggolf.android.editor.Components.CustomEditText _EditText = (donggolf.android.editor.Components.CustomEditText) _childRow.findViewById(R.id.txtText);
    ...
    public String getTextFromListItem(View row) {
        donggolf.android.editor.Components.CustomEditText _text = (donggolf.android.editor.Components.CustomEditText) row.findViewById(R.id.txtText);
        ...
        if (currentFocus == EditorType.UL_LI && !isOrdered) {
            ...
            if (view != null) {
                ...
                if (type == EditorType.INPUT) {
                    String text = ((donggolf.android.editor.Components.CustomEditText) view).getText().toString();  //get the text, if not null, replace it with list item
    ...
    public int getIndexOnEditorByEditText(donggolf.android.editor.Components.CustomEditText customEditText) {
    ...
    public donggolf.android.editor.Components.CustomEditText setFocusToSpecific(donggolf.android.editor.Components.CustomEditText customEditText) {
        TableRow tableRow = (TableRow) customEditText.getParent();
        TableLayout tableLayout = (TableLayout) tableRow.getParent();
        int indexOnTable = tableLayout.indexOfChild(tableRow);
        if (indexOnTable == 0) {
        ...
        if (prevRow != null) {
            donggolf.android.editor.Components.CustomEditText editText = (CustomEditText) tableRow.findViewById(R.id.txtText);
            if (editText.requestFocus()) {
                editText.setSelection(editText.getText().length());
            }
            return editText;
